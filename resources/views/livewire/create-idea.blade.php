...
<form wire:submit.prevent="createIdea" action="#" method="POST" class="space-y-4 px-4 py-6">
    <div>
        <input wire:model.defer="title" required name="title" type="text" class="w-full text-sm bg-gray-100 rounded-xl border-none placeholder-gray-900 px-4 py-2" placeholder="Your idea?">
        @error('title')
        <p>{{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        <textarea wire:model.defer="description" required name="description" id="idea_desc" cols="30" rows="4" class="w-full bg-gray-100 rounded-xl border-none placeholder-gray-900 text-sm px-4 py-2" placeholder="describe your idea further"></textarea>
